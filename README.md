...
- Plan settings include `View starting week` to hide old columns without changing scheduling. Use a label such as `26.21`, or a number such as `5` to show the current week plus five earlier weeks. If the cutoff would split a sprint, it snaps back to the sprint start. Incomplete external dependencies before the visible window remain visible at the left edge, with their original due week in the tooltip.
...
- Task/category colors appear only on weeks where the active allocation view shows task resources.
- Task status can color timeline cells, but empty cells stay uncolored. Green colors past non-zero cells green, Amber colors the current non-zero week bright yellow, Red colors the current non-zero week bright red, and Completed colors only non-zero task cells green while freezing history. Non-zero is based on the value shown in the active allocation view.
...
- External dependencies can optionally relate to a task for visual context. Hovering an external dependency highlights the non-zero timeline cells for that related task and for any task/category successors that hard-depend on the external dependency.
...
- The total effort row shows assigned/capacity per week. Assigned follows the active resource/effective view, while capacity is the resolved raw resource count. Click a week header or total-effort cell to edit week resources, working days, and vacation days in the week panel.
...
- Prerequisites inherit the urgency of the higher-priority work they unlock, so a low-priority prerequisite can be scheduled before unrelated lower-priority work.
