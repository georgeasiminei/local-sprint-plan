...
```json
{
  ...
  "status": "green",            // optional health status: green, amber, or red; omitted/null = none
  "resourceOverrides": [
...
Task vacation days are source rules. They are fractional-capable person-days that reduce only the selected task's contribution in that week.
Task status is a lightweight display marker. `none` is the default and is omitted from compact URL state; `green`, `amber`, and `red` persist as source task fields. Completed status is represented by the existing `completed` and `completedIntervals` fields because it freezes schedule history.
...
### 4.1 Core Rules
1. Tasks are sorted by effective scheduling priority (ascending — lower number = higher priority). A task's effective scheduling priority is the minimum of its own priority and the effective priorities of successor tasks it unlocks, so prerequisites for high-priority work are scheduled before unrelated lower-priority work.
...
5. **Vacation person-days reduce capacity:** `effectiveCapacity = max(0, workingDayAdjustedCapacity − planVacationDays / 5 − categoryVacationDays / 5)`. Vacation person-days accept fractional values such as `2.5`. Example: 4 resources and 10 plan vacation days gives 2 effective resources in that week. Example: 4 resources and 5 category vacation days gives 3 effective resources for that category in that week. Plan vacation applies to every task in the week, category vacation applies only to tasks in that category, and task vacation applies only to the selected task. Like working-day adjustments, plan and category scoped vacation also scale each affected task's weekly resource cap and task resource rules so the reduction is distributed across all affected activities rather than falling only on lower-priority work. Task vacation subtracts directly from only that task's effective allocation; for example, 2 allocated resources and 2.5 task vacation days produces 1.5 effective resources for that task in that week. When task vacation fully absorbs a task's raw allocation, that raw allocation remains reserved with zero effective effort so lower-priority tasks cannot backfill the absence and hide the lost capacity.
6. **Effective and raw allocations remain visible:** The timeline can show raw resource allocation or effective resource allocation. The total effort row displays effective allocation/raw week capacity for each week.
...
8. **Task resource rules:** A task resource override applies from its `weekIndex` onward and limits that task's weekly allocation until the task completes or another override starts. The timeline marks both the week where the rule starts and later weeks where the rule is inherited.
...
- **Task color:** Category and/or task colors are applied only to task cells where the current allocation view displays resources, not to the full row. A task created while a category is selected inherits that category and color by default.
- **Task status color:** Task status is selected from `None`, `Green (on track)`, `Amber (risk of delay)`, `Red (requires attention)`, and `Completed`. Empty task cells stay uncolored for every status. None uses normal task/category coloring. Green colors past-week non-zero task cells green. Amber colors only the current-week non-zero cell bright yellow. Red colors only the current-week non-zero cell bright red. Completed freezes the task as before and colors only non-zero task cells green. Non-zero is based on the value displayed in the current allocation view, so raw resource view can color a raw-reserved cell even if effective effort is zero.
...
- **Total effort row:** Displayed below task rows. Each week shows `x/y`, where `x` is calculated effective effort for that week and `y` is the resolved raw resource capacity for that week. The capacity value comes from week resources and inheritance, not from summing task cells, so display rounding in task rows cannot make the total capacity drift.
- **External dependency markers:** Expected external inputs are rendered as thin full-height deadline lines on the border after the due week. These lines use red for overdue incomplete markers, dark grey for future incomplete markers, yellow for partially complete markers, and green for complete markers. Free-text boxes are displayed in a dedicated dependency lane below the task table so they do not cover schedule cells. Where space allows, same-week dependency boxes are centered on the deadline line and stack vertically; near edges they may fall to the available side / shrink as needed instead of escaping the scrollable timeline. Compact boxes may truncate long text, with the full note and planning due week shown on hover. When old weeks are hidden, incomplete external dependencies from before the first visible week stay visible at the left boundary; completed hidden dependencies remain hidden.
- **External dependency hover highlighting:** Hovering or keyboard-focusing an external dependency box highlights only non-zero timeline cells for its optional related task. If that same external dependency is also a hard internal dependency predecessor, non-zero cells for the dependent task or every task in the dependent category are highlighted as well. The related-task highlight is visual-only and does not affect scheduling.
...
- Fields: name, category, priority, estimate (one decimal), notes, highlight color, earliestStartWeek, maxResources (one-decimal numeric input; empty = no limit), plus a task status dropdown with None, Green, Amber, Red, and Completed
- Dependencies list (incoming + outgoing) with add/remove controls
- Per-week allocation table (override individual weeks)
- Marking a task completed freezes its historical resource intervals. The Completed option is available only when the task is historical or currently in its final execution week, or when it is already completed. Tasks older than three weeks are auto-completed on load; if a timeframe change moves them back into the future, the completion data is removed.

### 5.3 Plan Settings Panel
- **Timeline start:** Edit planning start year and planning start week.
- **View starting week:** Optional view-only old-column cutoff. Accepts `YY.WW`/`YYYY.WW` or a number of weeks before the current week. Incomplete external dependencies before the cutoff stay visible as left-boundary markers.
...
- **Vacation days:** Person-days of absence, including fractional values such as `2.5`. Multiple scoped entries can coexist in the same week, such as 10 days for one category plus 3 days for one task and 2.5 days for another task. Entire-plan vacation days reduce every task's contribution; category vacation days reduce only tasks in the selected category; task vacation days reduce only the selected task.
