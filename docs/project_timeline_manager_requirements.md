...
6. **Effective and raw allocations remain visible:** The timeline can show raw resource allocation or effective resource allocation. In resource allocation view, the total effort row displays raw assigned allocation/raw week capacity. In effective resource view, it displays effective assigned allocation/raw week capacity.
...
- **Total effort row:** Displayed below task rows. Each week shows `x/y`, where `x` follows the active allocation view and `y` is the resolved raw resource capacity for that week. In resource allocation view, `x` is raw assigned resources; in effective resource view, `x` is calculated effective effort. The capacity value comes from week resources and inheritance, not from summing task cells, so display rounding in task rows cannot make the total capacity drift.
