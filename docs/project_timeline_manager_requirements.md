...
- **Task status color:** Task status is selected from `None`, `Green (on track)`, `Amber (risk of delay)`, `Red (requires attention)`, and `Completed`. None uses normal task/category coloring. Green colors all past-week task cells green. Amber colors only the current-week cell bright yellow. Red colors only the current-week cell bright red. Completed freezes the task as before and colors only non-zero task cells green.
