...
- `View starting week` is a render-only old-column cutoff. It accepts an absolute planning week label such as `26.21` or a relative number such as `5`, meaning current week plus five earlier weeks. The cutoff is snapped backward to the containing sprint start so visible sprint headers remain whole. Incomplete external dependencies before the visible window stay visible at the left boundary with their original planning due week in the tooltip.
- External dependencies are deadline markers with notes, status, and an optional visual-only related task, rendered as thin full-height timeline lines with editable note boxes in a lane below the table. Deadline weeks are edited as planning labels such as `26.12`. Incomplete past-due markers are red, incomplete future markers are dark grey, partial markers are yellow, and completed markers are green. Completed dependencies hidden by the view starting week remain hidden.
...
- External dependency hover highlighting is render-only. The highlight set is derived from the external dependency's optional `relatedTaskId` plus hard internal dependencies where that external dependency is the predecessor, but only non-zero timeline cells are visually highlighted. Only hard dependencies affect scheduling.
...
- Task resource edits are source rules that apply from the edited week onward. Timeline cells distinguish the week where a rule starts from later weeks where that same rule is inherited.
...
- Task/category colors render only in cells where the current allocation view displays allocated resources.
...
- The scheduler orders work by effective scheduling priority. A prerequisite inherits the urgency of higher-priority successor work it unlocks, so independent lower-priority work does not delay a higher-priority delivery chain.
